...
def load_risk_model() -> Tuple[Optional[object], Optional[list]]:
    ...
    # pickle resolves classes from the module they were defined in at pickle
    # time.  The risk model was pickled with PCABasedRiskScorer and
    # EmpiricalCorrelationScorer living in __main__ (a training script).
    # In Streamlit's multi-page runner sys.modules['__main__'] points to the
    # Streamlit process itself, not app.py, so pickle cannot find the classes
    # there.  We inject the stubs directly into __main__ before unpickling so
    # pickle's attribute lookup succeeds regardless of which page is active.
    _main = sys.modules.get("__main__")
    if _main is not None:
        if not hasattr(_main, "PCABasedRiskScorer"):
            _main.PCABasedRiskScorer = PCABasedRiskScorer
        if not hasattr(_main, "EmpiricalCorrelationScorer"):
            _main.EmpiricalCorrelationScorer = EmpiricalCorrelationScorer
...
def predict_stock_scores(
    df: pd.DataFrame, model: object, feature_names: list
) -> Dict[str, float]:
    """
